...
adjacency.image <- function(edges,required.groups=c(0),cex.axis=1) {
  ...
  axis(2,seq_len(n),rev(nodes),las=2,cex.axis=cex.axis) #J. Reum - I added the rev to get the lables in the right order on the fig. 
...
########################################################################
# If we instead supply the A matrix, instead of edges obtained from an #
#


#A_mat<-as.matrix(read.csv("MBSD_AdjMatrix.csv", row.names=1))

community.samplerAdj <- function(A_mat,required.groups=c(0)) {
  n.nodes <- dim(A_mat)[1]
  weight.labels <- rownames(A_mat)
  n.edges <- sum(A_mat!=0)

  W <- sign(A_mat)
  
  ## indices of non-zero matrix entries
  lower <- ifelse(  W[k.edges]==-1 ,-1L,0L)
  upper <- ifelse(  W[k.edges]==1 ,1L,0L)
  ...
  # Dealing with uncertain edges will, I think, be easiest, if a second matrix idenitcal to A_mat
  # in dimensions is loaded that indicates the uncertain links. 
  # For now, I'm assuming that all links in A_mat are certain 
...
###Added by Jon Reum (from Raymond et al. 2011)
